...
from hadron.models.resume import (
    OVERRIDE_NODE_MAP,
    PIPELINE_NODE_ORDER,
    pick_resume_node,
)
...
# ---------------------------------------------------------------------------
# pick_resume_node — single overrides
# ---------------------------------------------------------------------------


class TestPickResumeNodeSingle:
    def test_rebase_clean(self) -> None:
        assert pick_resume_node({"rebase_clean": True}) == "rebase"

    def test_review_passed(self) -> None:
        assert pick_resume_node({"review_passed": True}) == "review"
...
class TestPickResumeNodeMultiple:
    def test_review_and_verification(self) -> None:
        ...
        assert pick_resume_node({"review_passed": True, "behaviour_verified": True}) == "review"

    def test_rebase_and_review(self) -> None:
        # rebase (index 7) > review (index 6)
        assert pick_resume_node({"rebase_clean": True, "review_passed": True}) == "rebase"

    def test_all_three(self) -> None:
        # rebase is latest
        result = pick_resume_node({
            "rebase_clean": True,
            "review_passed": True,
...
# ---------------------------------------------------------------------------
# pick_resume_node — edge cases
# ---------------------------------------------------------------------------


class TestPickResumeNodeEdgeCases:
    def test_empty_dict(self) -> None:
        assert pick_resume_node({}) == "paused"

    def test_unrecognized_keys_only(self) -> None:
        assert pick_resume_node({"unknown_key": True, "another": 42}) == "paused"

    def test_mix_recognized_and_unrecognized(self) -> None:
        result = pick_resume_node({"garbage": True, "review_passed": True})
        assert result == "review"

    def test_false_values_still_map(self) -> None:
        """The function checks key presence, not truthiness."""
        assert pick_resume_node({"rebase_clean": False}) == "rebase"

    def test_none_value_still_maps(self) -> None:
        assert pick_resume_node({"review_passed": None}) == "review"
