...
from hadron.pipeline.nodes import (
    NodeContext, RepoInfo, emit_cost_update, make_agent_event_emitter,
...
"""
        ...
        logger.exception("Rebase node crashed (CR %s): %s", cr_id, exc)
        await ctx.event_bus.emit(PipelineEvent(
            cr_id=cr_id, event_type=EventType.STAGE_COMPLETED, stage="rebase",
            data={"error": str(exc)},
        ))
        return {
            "current_stage": "rebase",
            "status": "paused",
            "error": f"Rebase node failed: {exc}",
            "stage_history": [{"stage": "rebase", "status": "error", "error": str(exc)}],
        }
