Feature: Live Activity Feed
  ...
  Scenario: Active CRs appear in the feed    Given two CRs are currently running
  ...
  Scenario: Feed updates in real-time    Given a CR is shown in the activity feed
    When the CR enters a new stage
    Then the stage label updates without a page refresh
    And the last event text updates

  Scenario: Cost updates stream in real-time    Given a CR is shown in the activity feed
